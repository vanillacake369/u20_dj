...
// 외부 공격 방지 기능
require_once __DIR__ . "/../../security/input_filtering.php";
/**
 * @vanillacake369
...
$searchValue = [];
$searchValue["coach_country"] = getSearchValue($_GET["coach_country"] ?? NULL);
$searchValue["coach_region"] = getSearchValue($_GET["coach_region"] ?? NULL);
$searchValue["coach_division"] = getSearchValue($_GET["coach_division"] ?? NULL);
$searchValue["coach_gender"] = getSearchValue($_GET["coach_gender"] ?? NULL);
$searchValue["coach_duty"] = getSearchValue($_GET["coach_duty"] ?? NULL);
// $searchValue["coach_schedule"] = getSearchValue($_GET["coach_schedule"] ?? NULL);
$searchValue["coach_name"] = getSearchValue($_GET["coach_name"] ?? NULL);

// 검색값이 있는지에 대한 검증
$hasSearched = hasSearchedValue($searchValue);
if ($hasSearched) {
    $hasSearchedCountry = hasSearchedValue($searchValue["coach_country"]);
    $hasSearchedRegion = hasSearchedValue($searchValue["coach_region"]);
    $hasSearchedDivision = hasSearchedValue($searchValue["coach_division"]);
    $hasSearchedGender = hasSearchedValue($searchValue["coach_gender"]);
    $hasSearchedDuty = hasSearchedValue($searchValue["coach_duty"]);
    // $hasSearchedSports = hasSearchedValue($searchValue["coach_schedule"]);
    $hasSearchedName = hasSearchedValue($searchValue["coach_name"]);
}
...
if ($hasSearched) {
    ...
    $hasSearchedDuty = cleanHex($hasSearchedDuty);
    ...
    $hasSearchedDuty = htmlspecialchars($hasSearchedDuty, ENT_QUOTES);
...
if ($hasSearched) {
    if ($hasSearchedCountry) {
        $uri_array["coach_country"] = $searchValue["coach_country"];
        array_push($bindarray, $searchValue["coach_country"]);
        array_push($keyword, "coach_country=?");
    }
    if ($hasSearchedRegion) {
        $uri_array["coach_region"] = $searchValue["coach_region"];
        array_push($bindarray, $searchValue["coach_region"]);
        array_push($keyword, "coach_region=?");
    }
    if ($hasSearchedDivision) {
        $uri_array["coach_division"] = $searchValue["coach_division"];
        array_push($bindarray, $searchValue["coach_division"]);
        array_push($keyword, "coach_division=?");
    }
    if ($hasSearchedGender) {
        $uri_array["coach_gender"] = $searchValue["coach_gender"];
        array_push($bindarray, $searchValue["coach_gender"]);
        array_push($keyword, "coach_gender=?");
    }
    if ($hasSearchedDuty) {
        $uri_array["coach_duty"] = $searchValue["coach_duty"];
        array_push($bindarray, $searchValue["coach_duty"]);
        array_push($keyword, "coach_duty=?");
    }
    // if ($hasSearchedSports) {
    //     $uri_array["coach_schedule"] = $searchValue["coach_schedule"];
    //     $isAttendingSports = "'\\\\b" . $searchValue["coach_schedule"] . "\\\\b'";
    //     $isAttendingSports = "coach_schedule REGEXP " . str_replace(' ', '', $isAttendingSports);
    //     array_push($keyword, $isAttendingSports);
    // }
    if ($hasSearchedName) {
        $uri_array["coach_name"] = $searchValue["coach_name"];
        array_push($bindarray, trim("%{$searchValue["coach_name"]}%"));
        array_push($keyword, "coach_name LIKE ?");
    }
...
$isCountrySelected = maintainSelected($_GET["coach_country"] ?? NULL);
$isRegionSelected = maintainSelected($_GET["coach_region"] ?? NULL);
$isDivisionSelected = maintainSelected($_GET["coach_division"] ?? NULL);
$isGenderSelected = maintainSelected($_GET["coach_gender"] ?? NULL);
$isDutySelected = maintainSelected($_GET["coach_duty"] ?? NULL);
// $isSportsSelected = maintainSelected($_GET["coach_schedule"] ?? NULL);