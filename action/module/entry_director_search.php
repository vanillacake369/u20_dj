...
$searchValue = [];
$searchValue["director_country"] = getSearchValue($_GET["director_country"] ?? NULL);
$searchValue["director_division"] = getSearchValue($_GET["director_division"] ?? NULL);
$searchValue["director_gender"] = getSearchValue($_GET["director_gender"] ?? NULL);
$searchValue["director_duty"] = getSearchValue($_GET["director_duty"] ?? NULL);
// $searchValue["director_schedule"] = getSearchValue($_GET["director_schedule"] ?? NULL);
$searchValue["director_name"] = getSearchValue($_GET["director_name"] ?? NULL);

// 검색값이 있는지에 대한 검증
$hasSearched = hasSearchedValue($searchValue);
if ($hasSearched) {
    $hasSearchedCountry = hasSearchedValue($searchValue["director_country"]);
    $hasSearchedDivision = hasSearchedValue($searchValue["director_division"]);
    $hasSearchedGender = hasSearchedValue($searchValue["director_gender"]);
    $hasSearchedDuty = hasSearchedValue($searchValue["director_duty"]);
    // $hasSearchedSports = hasSearchedValue($searchValue["director_schedule"]);
    $hasSearchedName = hasSearchedValue($searchValue["director_name"]);
}
...
if ($hasSearched) {
    if ($hasSearchedCountry) {
        $uri_array["director_country"] = $searchValue["director_country"];
        array_push($bindarray, $searchValue["director_country"]);
        array_push($keyword, "director_country=?");
    }
    if ($hasSearchedDivision) {
        $uri_array["director_division"] = $searchValue["director_division"];
        array_push($bindarray, $searchValue["director_division"]);
        array_push($keyword, "director_division=?");
    }
    if ($hasSearchedGender) {
        $uri_array["director_gender"] = $searchValue["director_gender"];
        array_push($bindarray, $searchValue["director_gender"]);
        array_push($keyword, "director_gender=?");
    }
    if ($hasSearchedDuty) {
        $uri_array["director_duty"] = $searchValue["director_duty"];
        array_push($bindarray, $searchValue["director_duty"]);
        array_push($keyword, "director_duty=?");
    }
    // if ($hasSearchedSports) {
    //     $uri_array["director_schedule"] = $searchValue["director_schedule"];
    //     $isAttendingSports = "'\\\\b" . $searchValue["director_schedule"] . "\\\\b'";
    //     $isAttendingSports = "director_schedule REGEXP " . str_replace(' ', '', $isAttendingSports);
    //     array_push($keyword, $isAttendingSports);
    // }
    if ($hasSearchedName) {
        $uri_array["director_name"] = $searchValue["director_name"];
        array_push($bindarray, trim("%{$searchValue["director_name"]}%"));
        array_push($keyword, "director_name LIKE ?");
    }
...
$isCountrySelected = maintainSelected($_GET["director_country"] ?? NULL);
$isDivisionSelected = maintainSelected($_GET["director_division"] ?? NULL);
$isGenderSelected = maintainSelected($_GET["director_gender"] ?? NULL);
$isDutySelected = maintainSelected($_GET["director_duty"] ?? NULL);
// $isSportsSelected = maintainSelected($_GET["director_schedule"] ?? NULL);