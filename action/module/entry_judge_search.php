...
$searchValue = [];
$searchValue["judge_country"] = getSearchValue($_GET["judge_country"] ?? NULL);
$searchValue["judge_division"] = getSearchValue($_GET["judge_division"] ?? NULL);
$searchValue["judge_gender"] = getSearchValue($_GET["judge_gender"] ?? NULL);
//$searchValue["judge_duty"] = getSearchValue($_GET["judge_duty"] ?? NULL);
$searchValue["judge_schedule"] = getSearchValue($_GET["judge_schedule"] ?? NULL);
$searchValue["judge_name"] = getSearchValue($_GET["judge_name"] ?? NULL);

// 검색값이 있는지에 대한 검증
$hasSearched = hasSearchedValue($searchValue);
if ($hasSearched) {
    $hasSearchedCountry = hasSearchedValue($searchValue["judge_country"]);
    $hasSearchedDivision = hasSearchedValue($searchValue["judge_division"]);
    $hasSearchedGender = hasSearchedValue($searchValue["judge_gender"]);
    //$hasSearchedDuty = hasSearchedValue($searchValue["judge_duty"]);
    $hasSearchedSports = hasSearchedValue($searchValue["judge_schedule"]);
    $hasSearchedName = hasSearchedValue($searchValue["judge_name"]);
}
...
if ($hasSearched) {
    if ($hasSearchedCountry) {
        $uri_array["judge_country"] = $searchValue["judge_country"];
        array_push($bindarray, $searchValue["judge_country"]);
        array_push($keyword, "judge_country=?");
    }
    if ($hasSearchedDivision) {
        $uri_array["judge_division"] = $searchValue["judge_division"];
        array_push($bindarray, $searchValue["judge_division"]);
        array_push($keyword, "judge_division=?");
    }
    if ($hasSearchedGender) {
        $uri_array["judge_gender"] = $searchValue["judge_gender"];
        array_push($bindarray, $searchValue["judge_gender"]);
        array_push($keyword, "judge_gender=?");
    }
    // if ($hasSearchedDuty) {
    //     $uri_array["judge_duty"] = $searchValue["judge_duty"];
    //     array_push($bindarray, $searchValue["judge_duty"]);
    //     array_push($keyword, "judge_duty=?");
    //}
    if ($hasSearchedSports) {
        $uri_array["judge_schedule"] = $searchValue["judge_schedule"];
        $isAttendingSports = "'\\\\b" . $searchValue["judge_schedule"] . "\\\\b'";
        $isAttendingSports = "judge_schedule REGEXP " . str_replace(' ', '', $isAttendingSports);
        array_push($keyword, $isAttendingSports);
    }
    if ($hasSearchedName) {
        $uri_array["judge_name"] = $searchValue["judge_name"];
        array_push($bindarray, trim("%{$searchValue["judge_name"]}%"));
        array_push($keyword, "judge_name LIKE ?");
    }
...
$isCountrySelected = maintainSelected($_GET["judge_country"] ?? NULL);
$isDivisionSelected = maintainSelected($_GET["judge_division"] ?? NULL);
$isGenderSelected = maintainSelected($_GET["judge_gender"] ?? NULL);
// $isDutySelected = maintainSelected($_GET["judge_duty"] ?? NULL);
$isSportsSelected = maintainSelected($_GET["judge_schedule"] ?? NULL);