    ...
    if (strpos($ruf, "O") !== false) { //성공여부 판별
      ...
      if($tempstore =='1'){
        ...
        if(($checkrow[0]??null)>0){
            $savequery ="UPDATE list_record SET record_pass='$pass',record_judge='$judge[0]',record_".$result_type1."_record='$high[$i]', record_memo=CONCAT(record_memo,' ".$memo[$j]."'),record_trial='$ruf' 
      ...
      if($comprecord[$j] != $best){ //기존 기록과 변경된 기록이 같은 지 비교
        if($row1['schedule_status']==='y'){ //경기가 끝났는 지 판단
          if($rerow[0]==='y'){
            ...
            $memo[$j]=change_worldrecord_dec($athlete_name[$j],$row[1],$best,0,$s_id,$check_round,$arr);
          }else{
