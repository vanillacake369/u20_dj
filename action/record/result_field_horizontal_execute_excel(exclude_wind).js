...
function get_match_result() {
    ...
    const header_data = document.getElementsByTagName("th");
...
function add_match_information(html, sport_name, round, judge_name, wind, memo) {
    const title = ["Judge", "Start Time", "Equipment", "Memo"];
    html += "<tr>"
...
function execute_excel() {
    ...
    html = html + '<style> table, th, td { border: 1px solid black; border-collapse: collapse; text-align: center; }</style>';
    html = html + "<table>"
