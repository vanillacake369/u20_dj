...
function get_match_result() {
  ...
  for (let i = 0; i < match_data.length; i++) {
    ...
    for (let j = 0; j < header_data.length; j++) {
      if (match_data[i] === undefined) athlete_data.push(" ");
      else if (match_data[i].children[j] === undefined) athlete_data.push(" ");
      else {
...
function execute_excel() {
  ...
  [header, match_result] = get_match_result();
  header = ["Rank", "Lane", "Name", "Passed", "Result", "Memo", "Record"];
