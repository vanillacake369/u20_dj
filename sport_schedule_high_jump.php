...
$schedule_sports = $rows['schedule_sports'];
    ...
    $schedule_gender = $rows['schedule_gender'];
  ...
  <div class="schedule_container">
    ...
    <div class="schedule schedule_flex">
      ...
      <form action="" method="post">
        ...
        <input name="sports" value="<?=$schedule_sports?>" hidden>
        <input name="gender" value="<?=$schedule_gender?>" hidden>
        ...
        <form action="" method="post">
          ...
          <table class="box_table">
            ...
            <tbody class="table_tbody De_tbody entry_table">
              ...
              if ($row['schedule_group'] == $k) {
                  ...
                  echo '<td><input placeholder="비고" type="text" name="bigo[]"  value="' .
                    ($row["record_memo"] ?? null) . '" maxlength="100" readonly /></td>';
                  //
                  echo '<tr id=col2';
                  if ($num % 2 == 0) echo ' class="Ranklist_Background">';
                  else echo ">";
                  ...
                  if (($row['record_new'] && null) == 'y') {
                    ...
                    if ((key($athletics) ?? null) === 'w') {
                      ...
                      echo '<td><input placeholder="" type="text" name="newrecord[]"  value="" maxlength="100" ath="';
                      if (isset($row["athlete_name"])) echo $row["athlete_name"]; else echo "";
                      echo '" sports=' . $sport_code . ' schedule_id="' . $schedule_id . '" record="' . $row["record_" . $result_type . "_record"] . '" readonly/></td>';
                    }
                  } else {
                    echo '<td><input placeholder="" type="text" name="newrecord[]"  value="" maxlength="100" ath="';
                    if (isset($row["athlete_name"])) echo $row["athlete_name"]; else echo "";
                    echo '" sports=' . $sport_code . ' schedule_id="' . $schedule_id . '" record="' . ($row["record_" . $result_type . "_record"] ?? null) . '" readonly/></td>';
                  }
                  $cnt++;
                }
              $j++;
          ...
          <div class="filed_BTN">
            ...
            <div>
              ...
              // 수정 권한, 생성 권한 둘 다 있는 경우에만 접근 가능
              if (authCheck($db, "authSchedulesUpdate") && authCheck($db, "authSchedulesCreate")) {
                echo '<button type="submit" class="defaultBtn BIG_btn BTN_Blue filedBTN" onclick="window.open(\'';
                if ($rows['schedule_sports'] == "polevault" || $rows['schedule_sports'] == "highjump") {
                  echo "/record/field_vertical_result_view.php?id=" . $schedule_id;
                } else if ($rows['schedule_sports'] == "longjump" || $rows['schedule_sports'] == "triplejump") {
                  echo "/record/field_horizontal_result_view.php?id=" . $schedule_id;
    ...
    <!-- <div class="BTNform">
      <button type="button" class="nextBTN BTN_blue2 defaultBtn" onclick="window.open('/sport_schedule_group_next.php?id=<?= $id ?>', 'window_name', 'width=800, height=750, location=no, status=no, scrollbars=yes')">다음
        조 편성</button>
