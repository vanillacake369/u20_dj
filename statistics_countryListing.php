<?php
    ...
    $sql = "SELECT
                country_name,    
                country_code,
                count(DISTINCT(IF(record_medal=10000,schedule_id,NULL))) as gold_total,
                count(DISTINCT(IF(record_medal=100,schedule_id,NULL))) as silver_total,
                count(DISTINCT(IF(record_medal=1,schedule_id,NULL))) as bronze_total,                   
                GROUP_CONCAT(concat(record_medal,',',record_schedule_id,',',schedule_gender)ORDER BY record_schedule_id)  AS result_medal,
    ...
    $sql_where = "WHERE country_name IS NOT NULL ";
                        ...
                        while ($row = mysqli_fetch_array($result)) {
                            ...
                            if (count($sports) > 1) {
                                for ($i = 0; $i <= count($sports); $i = $i + 3) {
                                    ...
                                    if ($s == 10000) {
                                        array_push($Match, $schedule_dic[$sports[$i + 1]]);
                                        array_push($Match, $sports[$i + 2]);
                                    }
                                    if ($s == 10000) {
                                        array_push($goldWonMatch, $schedule_dic[$sports[$i + 1]]);
                                        array_push($goldWonMatch_gender, $sports[$i + 2]);
                                    }
                                    if ($s == 100) {
                                        array_push($silverWonMatch, $schedule_dic[$sports[$i + 1]]);
                                        array_push($silverWonMatch_gender, $sports[$i + 2]);
                                    }
                                    if ($s == 1) {
                                        array_push($bronzeWonMatch, $schedule_dic[$sports[$i + 1]]);
                                        array_push($bronzeWonMatch_gender, $sports[$i + 2]);
                                    }
                                    if ($s == 10000  || $s == 100 || $s == 1) {
                                        array_push($totalMatch, $schedule_dic[$sports[$i + 1]]);
