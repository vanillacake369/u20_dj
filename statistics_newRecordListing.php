                    ...
                    <form action="" name="judge_searchForm" method="get" class="searchForm pageArea">
                        <div class="page_size">
                            <select name="entry_size" onchange="changeTableSize(this);" id="changePageSize" class="changePageSize">
                                ...
                                <?php
                                    $get_count_sql = "SELECT COUNT(*) AS worldrecord_count FROM list_worldrecord
                                    INNER JOIN
                                        list_sports ON worldrecord_sports =sports_code
                                    INNER JOIN
                                        list_country ON worldrecord_country_code=country_code AND worldrecord_datetime >= '2015-01-01' ;";
                                    $count_result = $db->query($get_count_sql);
                                    $count_row = mysqli_fetch_array($count_result);
                                    $size_of_all = $count_row["worldrecord_count"];
