...
GROUP_CONCAT(CONCAT(IF(record_medal=10000,record_live_record,NULL))ORDER BY record_medal DESC)  AS gold_record,
GROUP_CONCAT(CONCAT(IF(record_medal=100,athlete_name,null))ORDER BY record_medal DESC)  AS silver_medal,
GROUP_CONCAT(CONCAT(IF(record_medal=100,record_live_record,null))ORDER BY record_medal DESC)  AS silver_record,
GROUP_CONCAT(CONCAT(IF(record_medal=1,athlete_name,null))ORDER BY record_medal DESC)  AS bronze_medal,
GROUP_CONCAT(CONCAT(IF(record_medal=1,record_live_record,null))ORDER BY record_medal DESC)  AS bronze_record
FROM list_athlete 
INNER JOIN list_record  ON record_athlete_id = athlete_id AND record_medal >=1
INNER JOIN list_schedule ON record_schedule_id = schedule_id
INNER JOIN list_sports ON sports_code = schedule_sports";
    
$sql_where = " WHERE record_medal >=1 ";
$sql_order = " ORDER BY sports_code, schedule_gender";
$sql_like = "";
$sql_group = "GROUP BY schedule_id";
